...
float tempSensorReadData()
{
  ...
  if (HAL_I2C_Master_Transmit(&hi2c3, I2C_ADDRESS, buf, 1, 1000) != HAL_OK)
    return 999;
  if (HAL_I2C_Master_Receive(&hi2c3, I2C_ADDRESS, buf, 2, 1000) != HAL_OK)
    return 999;

  uint16_t tempInt = buf[1];
  tempInt += (buf[0] & 0x7F)<<8;
  tempInt = (tempInt >> 7);
