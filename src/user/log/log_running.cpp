...
void LogRunning::task()
{
  while (1) {
    ...
    if (type_ == RunTypeLog) {
      LastReasonRun eventType = (LastReasonRun)ksu.getValue(CCS_LAST_RUN_REASON_TMP);
...
void LogRunning::add()
{
  ...
  *(float*)(buffer+4) = parameters.getValue(CCS_RESISTANCE_ISOLATION);
  write(buffer, 256);
  ...
  for (int i = 0; i < ADC_POINTS_NUM/8; ++i) {
    ...
    for (int j = 0; j < 8; ++j) {
      ...
      *(float*)(buffer+16+j*32) = parameters.getValue(CCS_MOTOR_COS_PHI_NOW);
